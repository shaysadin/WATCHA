...
const MovieCard: React.FC<MovieCardProps> = ({ data }) => {
    ...
    return (
      ...
      ">
         <video onClick={() => openModal(data?.id)} className={`cursor-pointer
          object-cover
          transition
          duration
          shadow-xl
          rounded-t-md
          w-full
          h-[23vh]`} autoPlay muted loop src={data?.videoUrl}></video>
        {/* <img onClick={() => openModal(data?.id)} src={data.thumbnailUrl} alt="Movie" draggable={false} className="
          cursor-pointer
          object-cover
          transition
        ...
        <div className="
          ...
          <div className="flex flex-row items-center gap-3">
            ...
            <FavoriteButton movieId={data.id} />
            <div onClick={() => openModal(data?.id)} className="cursor-pointer ml-auto group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center transition hover:border-neutral-300">
              <TiInfoLargeOutline className="text-white group-hover/item:text-neutral-300 w-4 lg:w-6" />
            </div>
          </div>
          <p className="text-green-400 font-semibold mt-4">
            New <span className="text-white">2023</span>
